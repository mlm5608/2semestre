...
.AddJwtBearer("JwtBearer", options => 
{
    options.TokenValidationParameters = new TokenValidationParameters
    {
        ...
        IssuerSigningKey = new SymmetricSecurityKey (System.Text.Encoding.UTF8.GetBytes("flmes-chave-autenticacao-webapi-dev")),

        //Valida tempo de expiração do token
        ClockSkew = TimeSpan.FromMinutes(5),

        //De onde está vindo (issuer)
        ValidIssuer = "webapi.filmes.miguel",

        //Para onde está indo (audience)
        ValidAudience = "webapi.filmes.miguel"
    };
}); 
